...
compile ()
{
  diag "info: building iw4-open-formats ($cfg)"

  sln="build/iw4-of.slnx"
  if ! test -f "$sln"; then
...
build ()
{
  ...
  diag "-------------------------------------------------------------------------"
  diag
  diag "About to perform fast path build of iw4-open-formats."
  diag
...
bootstrap ()
{
  ...
  diag "-------------------------------------------------------------------------"
  diag
  diag "About to bootstrap iw4-open-formats build environment and compile client."
  diag
  diag "Build configuration: $cfg"
  diag "Wine prefix:         $wn_dir/"
  diag "MSVC install dir:    $mi_dir/"
