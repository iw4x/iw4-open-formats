...
#define READ_BOOL_LB_FROM_JSON(x) bool x = jsonStateBitEntry[#x].Get<bool>()
	...
	) const
	{
		...
		for (auto index = 0u; index < count; index++)
		{
			...
			const auto depthTest = (entry.flags.loadbit1 & native::GFXS1_DEPTHTEST_DISABLE) ? -1 : (entry.flags.loadbit1 & native::GFXS1_DEPTHTEST_MASK) >> native::GFXS1_DEPTHTEST_SHIFT;
