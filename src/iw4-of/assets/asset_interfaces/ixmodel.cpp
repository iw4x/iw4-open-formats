...
namespace iw4of::interfaces
{
	bool ixmodel::write_internal(const native::XAssetHeader& header) const
	{
		...
		if (asset->physPreset)
		{
			buffer.save_object(*asset->physPreset);
	...
	void ixmodel::write(const native::XSurface* surf, utils::stream* buffer) const
	{
		...
		{
			buffer->save_array_if_not_existing(surf->vertInfo.vertsBlend, surf->vertInfo.vertCount[0] + (surf->vertInfo.vertCount[1] * 3) + (surf->vertInfo.vertCount[2] * 5) + (surf->vertInfo.vertCount[3] * 7));
		}
