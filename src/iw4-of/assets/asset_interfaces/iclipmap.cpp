...
#define IW4X_CLIPMAP_VERSION 3
	...
	void* iclipmap::read_internal(const std::string& _name) const
	{
		...
		try
		{
			...
			clipmap->staticModelList = clipmap->numStaticModels == 0 ? nullptr : local_allocator.allocate_array<native::cStaticModel_s>(clipmap->numStaticModels);
			...
			clipmap->leafbrushNodes = clipmap->leafbrushNodesCount == 0 ? nullptr : local_allocator.allocate_array<native::cLeafBrushNode_s>(clipmap->leafbrushNodesCount);
			...
			clipmap->partitions = clipmap->partitionCount == 0 ? nullptr : local_allocator.allocate_array<native::CollisionPartition>(clipmap->partitionCount);
			...
			for (size_t i = 0; i < 2; i++)
			{
				...
				clipmap->dynEntCollList[i] = clipmap->dynEntCount[i] == 0 ? nullptr : local_allocator.allocate_array<native::DynEntityColl>(clipmap->dynEntCount[i]);
				clipmap->dynEntPoseList[i] = clipmap->dynEntCount[i] == 0 ? nullptr : local_allocator.allocate_array<native::DynEntityPose>(clipmap->dynEntCount[i]);
				clipmap->dynEntDefList[i] = clipmap->dynEntCount[i] == 0 ? nullptr : local_allocator.allocate_array<native::DynEntityDef>(clipmap->dynEntCount[i]);
