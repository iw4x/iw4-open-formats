...
#define IW4X_CLIPMAP_VERSION 3
  ...
  void* iclipmap::read_internal(const std::string& _name) const
  {
    ...
    try
    {
      ...
      clipmap->brushsides = clipmap->numBrushSides == 0 ? nullptr : local_allocator.allocate_array<native::cbrushside_t>(clipmap->numBrushSides);
      ...
      clipmap->aabbTrees = clipmap->aabbTreeCount == 0 ? nullptr : local_allocator.allocate_array<native::CollisionAabbTree>(clipmap->aabbTreeCount);
      ...
      for (size_t i = 0; i < 2; i++)
      {
        ...
        for (size_t j = 0; j < clipmap->dynEntCount[i]; j++)
        {
          ...
          entity->destroyFx = json_entity["destroyFx"].IsString()
                                  ...
                                  : nullptr;
          entity->physPreset = json_entity["physPreset"].IsString()
                                   ? find<native::PhysPreset>(native::XAssetType::ASSET_TYPE_PHYSPRESET, json_entity["physPreset"].GetString())
                                   : nullptr;
