...
#define IW4X_CLIPMAP_VERSION 3
  ...
  void* iclipmap::read_internal(const std::string& _name) const
  {
    ...
    try
    {
      ...
      clipmap->name = local_allocator.duplicate_string(
          clipmap_json["name"].Get<std::string>());
      clipmap->isInUse = clipmap_json["isInUse"].Get<int32_t>();

      // planes
      clipmap->planeCount = clipmap_json["planes"].Size();
      clipmap->planes = clipmap->planeCount == 0
                            ? nullptr
                            : local_allocator.allocate_array<native::cplane_s>(
                                  clipmap->planeCount);
      ...
      for (size_t i = 0; i < clipmap->planeCount; i++)
      {
        ...
        utils::json::copy_array(
            plane->normal, clipmap_json["planes"][i]["normal"], 3);
      ...
      clipmap->staticModelList =
          clipmap->numStaticModels == 0
              ? nullptr
              : local_allocator.allocate_array<native::cStaticModel_s>(
      ...
      for (size_t i = 0; i < clipmap->numStaticModels; i++)
      {
        ...
        model->xmodel = find<native::XModel>(
            native::XAssetType::ASSET_TYPE_XMODEL, model_name);
        ...
        {
          utils::json::copy_array(
              model->invScaledAxis[j], json_model["invScaledAxis"][j], 3);
        }
      ...
      clipmap->materials =
          clipmap->numMaterials == 0
              ? nullptr
              : local_allocator.allocate_array<native::ClipMaterial>(
      ...
      for (size_t i = 0; i < clipmap->numMaterials; i++)
      {
        ...
        material->name = local_allocator.duplicate_string(
            json_material["name"].Get<std::string>());
      ...
      clipmap->brushsides =
          clipmap->numBrushSides == 0
              ? nullptr
              : local_allocator.allocate_array<native::cbrushside_t>(
      ...
      for (size_t i = 0; i < clipmap->numBrushSides; i++)
      {
        ...
        brushside->plane = &clipmap->planes[std::stoi(
            json_brushside["plane"].Get<std::string>().substr(1))];
        brushside->materialNum = json_brushside["materialNum"].Get<uint16_t>();
        brushside->firstAdjacentSideOffset =
            json_brushside["firstAdjacentSideOffset"].Get<char>();
        brushside->edgeCount = json_brushside["edgeCount"].Get<char>();
      }

      clipmap->numBrushEdges = clipmap_json["brushEdges"].Size();
      clipmap->brushEdges =
          clipmap->numBrushEdges == 0
              ? nullptr
              : local_allocator.allocate_array<uint8_t>(clipmap->numBrushEdges);
      utils::json::copy_array(clipmap->brushEdges, clipmap_json["brushEdges"]);

      clipmap->numNodes = clipmap_json["nodes"].Size();
      clipmap->nodes = clipmap->numNodes == 0
                           ? nullptr
                           : local_allocator.allocate_array<native::cNode_t>(
                                 clipmap->numNodes);
      ...
      for (size_t i = 0; i < clipmap->numNodes; i++)
      {
        ...
        assert(clipmap->planes);
        node->plane = &clipmap->planes[std::stoi(
            json_node["plane"].Get<std::string>().substr(1))];
      ...
      clipmap->leafs = clipmap->numLeafs == 0
                           ? nullptr
                           : local_allocator.allocate_array<native::cLeaf_t>(
                                 clipmap->numLeafs);
      ...
      for (size_t i = 0; i < clipmap->numLeafs; i++)
      {
        ...
        leaf->firstCollAabbIndex =
            json_leaf["firstCollAabbIndex"].Get<uint16_t>();
      ...
      clipmap->leafbrushNodes =
          clipmap->leafbrushNodesCount == 0
              ? nullptr
              : local_allocator.allocate_array<native::cLeafBrushNode_s>(
                    clipmap->leafbrushNodesCount);

      // Leafbrushes
      clipmap->numLeafBrushes = clipmap_json["leafbrushes"].Size();
      clipmap->leafbrushes = clipmap->numLeafBrushes == 0
                                 ? nullptr
                                 : local_allocator.allocate_array<uint16_t>(
                                       clipmap->numLeafBrushes);
      utils::json::copy_array(clipmap->leafbrushes,
                              clipmap_json["leafbrushes"]);

      clipmap->numLeafSurfaces = clipmap_json["leafsurfaces"].Size();
      clipmap->leafsurfaces = clipmap->numLeafSurfaces == 0
                                  ? nullptr
                                  : local_allocator.allocate_array<uint32_t>(
                                        clipmap->numLeafSurfaces);
      utils::json::copy_array(clipmap->leafsurfaces,
                              clipmap_json["leafsurfaces"]);

      clipmap->vertCount = clipmap_json["verts"].Size();
      clipmap->verts = clipmap->vertCount == 0
                           ? nullptr
                           : local_allocator.allocate_array<native::vec3_t>(
                                 clipmap->vertCount);
      ...
      for (size_t i = 0; i < clipmap->leafbrushNodesCount; i++)
      {
        ...
        {
          int32_t index =
              std::stoi(json_lbn["data"].Get<std::string>().substr(1));
        ...
        else
        {
          ...
          utils::json::copy_array(lbn->data.children.childOffset,
                                  json_lbn["data"]["childOffset"]);
        }
      }
      ...
      clipmap->triIndices =
          clipmap->triCount == 0
              ? nullptr
              : local_allocator.allocate_array<uint16_t>(clipmap->triCount * 3);

      for (size_t i = 0; i < clipmap->triCount * 3; i += 3)
      {
        utils::json::copy_array(&clipmap->triIndices[i],
                                clipmap_json["triIndices"][i / 3]);
      }
      ...
      clipmap->triEdgeIsWalkable =
          walkableCount == 0
              ? nullptr
              : local_allocator.allocate_array<uint8_t>(walkableCount);
      utils::json::copy_array(clipmap->triEdgeIsWalkable,
                              clipmap_json["triEdgeIsWalkable"]);

      // Borders
      clipmap->borderCount = clipmap_json["borders"].Size();
      clipmap->borders =
          clipmap->borderCount == 0
              ? nullptr
              : local_allocator.allocate_array<native::CollisionBorder>(
                    clipmap->borderCount);
      ...
      clipmap->partitions =
          clipmap->partitionCount == 0
              ? nullptr
              : local_allocator.allocate_array<native::CollisionPartition>(
      ...
      for (size_t i = 0; i < clipmap->partitionCount; i++)
      {
        ...
        partition->firstVertSegment =
            json_partition["firstVertSegment"].Get<uint8_t>();
        ...
        if (partition->borderCount > 0)
        {
          ...
          auto index = std::stoi(
              json_partition["firstBorder"].Get<std::string>().substr(1));
      ...
      clipmap->aabbTrees =
          clipmap->aabbTreeCount == 0
              ? nullptr
              : local_allocator.allocate_array<native::CollisionAabbTree>(
      ...
      clipmap->cmodels = clipmap->numSubModels == 0
                             ? nullptr
                             : local_allocator.allocate_array<native::cmodel_t>(
      ...
      // Brushes
      clipmap->numBrushes =
          static_cast<uint16_t>(clipmap_json["brushes"].Size());
      clipmap->brushes = clipmap->numBrushes == 0
                             ? nullptr
                             : local_allocator.allocate_array<native::cbrush_t>(
                                   clipmap->numBrushes);
      for (size_t i = 0; i < clipmap->numBrushes; i++)
      {
        ...
        if (brush->numsides)
        {
          // Always consecutive
          auto index =
              std::stoi(json_brush["firstSide"].Get<std::string>().substr(1));
        ...
        {
          auto index = std::stoi(
              json_brush["baseAdjacentSide"].Get<std::string>().substr(1));
        ...
        {
          utils::json::copy_array(brush->axialMaterialNum[x],
                                  json_brush["axialMaterialNum"][x]);
          utils::json::copy_array(brush->firstAdjacentSideOffsets[x],
                                  json_brush["firstAdjacentSideOffsets"][x]);
          utils::json::copy_array(brush->edgeCount[x],
                                  json_brush["edgeCount"][x]);
        }
      }

      assert(clipmap_json["brushes"].Size() ==
             clipmap_json["brushBounds"].Size());
      clipmap->brushBounds =
          local_allocator.allocate_array<native::Bounds>(clipmap->numBrushes);
      for (size_t i = 0; i < clipmap->numBrushes; i++)
      {
        clipmap->brushBounds[i] =
            utils::json::read_bounds(clipmap_json["brushBounds"][i]);
      }

      assert(clipmap_json["brushes"].Size() ==
             clipmap_json["brushContents"].Size());
      clipmap->brushContents =
          local_allocator.allocate_array<int32_t>(clipmap->numBrushes);
      utils::json::copy_array(clipmap->brushContents,
                              clipmap_json["brushContents"]);

      auto& json_ents = clipmap_json["mapEnts"];
      if (!json_ents.IsNull())
      {
        auto ents_name = json_ents["name"].GetString();
        clipmap->mapEnts = find<native::MapEnts>(
            native::XAssetType::ASSET_TYPE_MAP_ENTS, ents_name);
        ...
        if (clipmap->mapEnts)
        {
          ...
          trigger->models =
              local_allocator.allocate_array<native::TriggerModel>(
                  trigger->count);

          for (size_t i = 0; i < trigger->count; i++)
          {
            trigger->models[i].contents =
                json_trigger["models"][i]["contents"].Get<int32_t>();
            trigger->models[i].hullCount =
                json_trigger["models"][i]["hullCount"].Get<uint16_t>();
            trigger->models[i].firstHull =
                json_trigger["models"][i]["firstHull"].Get<uint16_t>();
          }

          trigger->hullCount = json_trigger["hulls"].Size();
          trigger->hulls = local_allocator.allocate_array<native::TriggerHull>(
              trigger->hullCount);
          for (size_t i = 0; i < trigger->hullCount; i++)
          {
            trigger->hulls[i].bounds =
                utils::json::read_bounds(json_trigger["hulls"][i]["bounds"]);

            trigger->hulls[i].contents =
                json_trigger["hulls"][i]["contents"].Get<int32_t>();
            trigger->hulls[i].firstSlab =
                json_trigger["hulls"][i]["firstSlab"].Get<uint16_t>();
            trigger->hulls[i].slabCount =
                json_trigger["hulls"][i]["slabCount"].Get<uint16_t>();
          }

          trigger->slabCount = json_trigger["slabs"].Size();
          trigger->slabs = local_allocator.allocate_array<native::TriggerSlab>(
              trigger->slabCount);
          for (size_t i = 0; i < trigger->slabCount; i++)
          {
            utils::json::copy_array(trigger->slabs[i].dir,
                                    json_trigger["slabs"][i]["dir"]);
            trigger->slabs[i].midPoint =
                json_trigger["slabs"][i]["midPoint"].Get<float>();
            trigger->slabs[i].halfSize =
                json_trigger["slabs"][i]["halfSize"].Get<float>();
          }

          // Stages
          clipmap->mapEnts->stageCount =
              static_cast<char>(json_ents["stages"].Size());
          clipmap->mapEnts->stages =
              local_allocator.allocate_array<native::Stage>(
                  clipmap->mapEnts->stageCount);
          for (auto i = 0; i < clipmap->mapEnts->stageCount; i++)
          {
            ...
            stage->name = local_allocator.duplicate_string(
                json_stage["name"].GetString());
            ...
            stage->sunPrimaryLightIndex =
                json_stage["sunPrimaryLightIndex"].Get<char>();
          }
        }
      }
      ...
      clipmap->smodelNodeCount =
          static_cast<uint16_t>(clipmap_json["smodelNodes"].Size());
      clipmap->smodelNodes =
          clipmap->smodelNodeCount == 0
              ? nullptr
              : local_allocator.allocate_array<native::SModelAabbNode>(
                    clipmap->smodelNodeCount);
      ...
      for (size_t i = 0; i < 2; i++)
      {
        ...
        clipmap->dynEntCount[i] =
            static_cast<uint16_t>(clipmap_json["dynEntities"][i].Size());

        auto& json_entities = clipmap_json["dynEntities"][i];
        clipmap->dynEntClientList[i] =
            clipmap->dynEntCount[i] == 0
                ? nullptr
                : local_allocator.allocate_array<native::DynEntityClient>(
                      clipmap->dynEntCount[i]);
        clipmap->dynEntCollList[i] =
            clipmap->dynEntCount[i] == 0
                ? nullptr
                : local_allocator.allocate_array<native::DynEntityColl>(
                      clipmap->dynEntCount[i]);
        clipmap->dynEntPoseList[i] =
            clipmap->dynEntCount[i] == 0
                ? nullptr
                : local_allocator.allocate_array<native::DynEntityPose>(
                      clipmap->dynEntCount[i]);
        clipmap->dynEntDefList[i] =
            clipmap->dynEntCount[i] == 0
                ? nullptr
                : local_allocator.allocate_array<native::DynEntityDef>(
                      clipmap->dynEntCount[i]);
        ...
        for (size_t j = 0; j < clipmap->dynEntCount[i]; j++)
        {
          ...
          entity->type = static_cast<native::DynEntityType>(
              json_entity["type"].Get<int32_t>());
          utils::json::copy_array(entity->pose.quat,
                                  json_entity["pose"]["quat"]);
          utils::json::copy_array(entity->pose.origin,
                                  json_entity["pose"]["origin"]);

          entity->xModel =
              json_entity["xModel"].IsString()
                  ? find<native::XModel>(native::XAssetType::ASSET_TYPE_XMODEL,
                                         json_entity["xModel"].GetString())
                  : nullptr;
          entity->brushModel = json_entity["brushModel"].Get<uint16_t>();
          entity->physicsBrushModel =
              json_entity["physicsBrushModel"].Get<uint16_t>();
          entity->destroyFx = json_entity["destroyFx"].IsString()
                                  ? find<native::FxEffectDef>(
                                        native::XAssetType::ASSET_TYPE_FX,
                                        json_entity["destroyFx"].GetString())
                                  : nullptr;
          entity->physPreset =
              json_entity["physPreset"].IsString()
                  ? find<native::PhysPreset>(
                        native::XAssetType::ASSET_TYPE_PHYSPRESET,
                        json_entity["physPreset"].GetString())
                  : nullptr;
          entity->health = json_entity["health"].Get<int32_t>();

          utils::json::copy_array(entity->mass.centerOfMass,
                                  json_entity["mass"]["centerOfMass"]);
          utils::json::copy_array(entity->mass.momentsOfInertia,
                                  json_entity["mass"]["momentsOfInertia"]);
          utils::json::copy_array(entity->mass.productsOfInertia,
                                  json_entity["mass"]["productsOfInertia"]);
  ...
  bool iclipmap::write_plaintext(const native::XAssetHeader& header) const
  {
    ...
    auto uchar_to_array = [&allocator](const uint8_t* array, int size)
    {
      ...
      for (auto i = 0; i < size; i++)
      {
        arr.PushBack(rapidjson::Value(static_cast<uint8_t>(array[i])),
                     allocator);
      }
    ...
    auto bounds_to_json =
        [&allocator, &float_array_to_json](const native::Bounds& bounds)
    {
      rapidjson::Value bounds_json(rapidjson::kObjectType);

      bounds_json.AddMember(
          "midPoint", float_array_to_json(bounds.midPoint, 3), allocator);
      bounds_json.AddMember(
          "halfSize", float_array_to_json(bounds.halfSize, 3), allocator);

      return bounds_json;
    };

    auto placement_to_json = [&allocator, &float_array_to_json](
                                 const native::GfxPlacement& placement)
    {
      rapidjson::Value placement_json(rapidjson::kObjectType);

      placement_json.AddMember(
          "quat", float_array_to_json(placement.quat, 4), allocator);
      placement_json.AddMember(
          "origin", float_array_to_json(placement.origin, 3), allocator);

      return placement_json;
    };

    auto leaf_to_json =
        [&allocator, &bounds_to_json](const native::cLeaf_t& leaf)
    {
      rapidjson::Value json_leave(rapidjson::kObjectType);

      json_leave.AddMember(
          "firstCollAabbIndex", leaf.firstCollAabbIndex, allocator);
    ...
    for (size_t i = 0; i < clip_map->numStaticModels; i++)
    {
      ...
      json_static_model.AddMember(
          "xmodel",
          static_model->xmodel == nullptr
              ? rapidjson::Value(rapidjson::kNullType)
              : RAPIDJSON_STR(static_model->xmodel->name),
          allocator);
      json_static_model.AddMember(
          "origin",
          float_array_to_json(static_model->origin,
                              ARRAYSIZE(static_model->origin)),
          allocator);
      ...
      for (size_t j = 0; j < ARRAYSIZE(static_model->invScaledAxis); j++)
      {
        inv_scaled_axis.PushBack(
            float_array_to_json(static_model->invScaledAxis[j],
                                ARRAYSIZE(static_model->invScaledAxis[j])),
            allocator);
      }
      ...
      json_static_model.AddMember(
          "absBounds", bounds_to_json(static_model->absBounds), allocator);
    ...
    for (size_t i = 0; i < clip_map->numMaterials; i++)
    {
      ...
      json_material.AddMember(
          "surfaceFlags", material->surfaceFlags, allocator);
    ...
    for (size_t i = 0; i < clip_map->numBrushSides; i++)
    {
      ...
      json_brush_side.AddMember(
          "materialNum", brush_side->materialNum, allocator);
      json_brush_side.AddMember("firstAdjacentSideOffset",
                                brush_side->firstAdjacentSideOffset,
                                allocator);
    ...
    for (size_t i = 0; i < clip_map->leafbrushNodesCount; i++)
    {
      ...
      json_leaf_brush_node.AddMember(
          "leafBrushCount", leaf_brush_node->leafBrushCount, allocator);
      json_leaf_brush_node.AddMember(
          "contents", leaf_brush_node->contents, allocator);

      if (leaf_brush_node->leafBrushCount > 0)
      {
        assert(leaf_brushes.contains(leaf_brush_node->data.leaf.brushes));
        auto index_str = std::format(
            "#{}", leaf_brushes[leaf_brush_node->data.leaf.brushes]);
        json_leaf_brush_node.AddMember(
            "data",
            RAPIDJSON_STR(mem_allocator.duplicate_string(index_str)),
            allocator);
      }
      else
      {
        ...
        data.AddMember(
            "range", leaf_brush_node->data.children.range, allocator);

        rapidjson::Value child_offset(rapidjson::kArrayType);
        for (size_t x = 0; x < 2; x++)
        {
          child_offset.PushBack(leaf_brush_node->data.children.childOffset[x],
                                allocator);
        }
    ...
    {
      json_vertices.PushBack(float_array_to_json(clip_map->verts[i], 3),
                             allocator);
    }
    ...
    for (size_t i = 0; i < clip_map->triCount * 3; i += 3)
    {
      ...
      json_tris.PushBack(ushort_to_array(&clip_map->triIndices[i], 3),
                         allocator);
    }
    ...
    {
      json_tris_walkable.PushBack(
          static_cast<uint8_t>(clip_map->triEdgeIsWalkable[i]), allocator);
    }
    ...
    for (size_t i = 0; i < clip_map->borderCount; i++)
    {
      ...
      json_collision_border.AddMember(
          "distEq",
          float_array_to_json(collision_border->distEq, 3),
          allocator);
      json_collision_border.AddMember(
          "zBase", collision_border->zBase, allocator);
      json_collision_border.AddMember(
          "zSlope", collision_border->zSlope, allocator);
      json_collision_border.AddMember(
          "start", collision_border->start, allocator);
      json_collision_border.AddMember(
          "length", collision_border->length, allocator);
    ...
    for (size_t i = 0; i < clip_map->partitionCount; i++)
    {
      ...
      json_collision_partition.AddMember(
          "triCount", collision_partition->triCount, allocator);
      json_collision_partition.AddMember(
          "firstVertSegment", collision_partition->firstVertSegment, allocator);
      json_collision_partition.AddMember(
          "firstTri", collision_partition->firstTri, allocator);
      json_collision_partition.AddMember(
          "borderCount", collision_partition->borderCount, allocator);

      if (collision_partition->borderCount)
      {
        auto index_str = str_duplicator.duplicate_string(
            std::format("#{}", borders[&collision_partition->borders[0]]));
        json_collision_partition.AddMember(
            "firstBorder", RAPIDJSON_STR(index_str), allocator);
      }
    ...
    for (size_t i = 0; i < clip_map->aabbTreeCount; i++)
    {
      ...
      json_aabbtree.AddMember(
          "midPoint", float_array_to_json(aabbtree->midPoint, 3), allocator);
      json_aabbtree.AddMember(
          "halfSize", float_array_to_json(aabbtree->halfSize, 3), allocator);
      json_aabbtree.AddMember(
          "materialIndex", aabbtree->materialIndex, allocator);
    ...
    for (size_t i = 0; i < clip_map->numSubModels; i++)
    {
      ...
      json_cmodel.AddMember(
          "bounds", bounds_to_json(cmodel->bounds), allocator);
    ...
    for (size_t i = 0; i < clip_map->numBrushes; i++)
    {
      ...
      json_brush.AddMember(
          "glassPieceIndex", brush->glassPieceIndex, allocator);
      ...
      {
        auto index_str = str_duplicator.duplicate_string(
            std::format("#{}", brush_sides[brush->sides]));
      ...
      {
        auto index_str = str_duplicator.duplicate_string(
            std::format("#{}", brush_edges[brush->baseAdjacentSide]));
        json_brush.AddMember(
            "baseAdjacentSide", RAPIDJSON_STR(index_str), allocator);
      }
      else
      {
        json_brush.AddMember("baseAdjacentSide",
                             rapidjson::Value(rapidjson::kNullType),
                             allocator);
      }
      ...
      for (size_t x = 0; x < 2; x++)
      {
        axial_material_num.PushBack(
            ushort_to_array(brush->axialMaterialNum[x], 3), allocator);
      }
      ...
      for (size_t x = 0; x < 2; x++)
      {
        first_adjacent_side_offsets.PushBack(
            uchar_to_array(brush->firstAdjacentSideOffsets[x], 3), allocator);
      }
      json_brush.AddMember(
          "firstAdjacentSideOffsets", first_adjacent_side_offsets, allocator);
    ...
    for (size_t i = 0; i < clip_map->numBrushes; i++)
    {
      json_brush_bounds.PushBack(bounds_to_json(clip_map->brushBounds[i]),
                                 allocator);
    }
    ...
    if (clip_map->mapEnts)
    {
      ...
      for (size_t i = 0; i < ents->trigger.count; i++)
      {
        ...
        json_trigger_model.AddMember(
            "contents", ents->trigger.models[i].contents, allocator);
        json_trigger_model.AddMember(
            "hullCount", ents->trigger.models[i].hullCount, allocator);
        json_trigger_model.AddMember(
            "firstHull", ents->trigger.models[i].firstHull, allocator);
      ...
      for (size_t i = 0; i < ents->trigger.hullCount; i++)
      {
        ...
        json_trigger_hull.AddMember(
            "bounds", bounds_to_json(ents->trigger.hulls[i].bounds), allocator);
        json_trigger_hull.AddMember(
            "contents", ents->trigger.hulls[i].contents, allocator);
        json_trigger_hull.AddMember(
            "slabCount", ents->trigger.hulls[i].slabCount, allocator);
        json_trigger_hull.AddMember(
            "firstSlab", ents->trigger.hulls[i].firstSlab, allocator);
      ...
      for (size_t i = 0; i < ents->trigger.slabCount; i++)
      {
        ...
        json_trigger_slab.AddMember(
            "dir",
            float_array_to_json(ents->trigger.slabs[i].dir, 3),
            allocator);
        json_trigger_slab.AddMember(
            "midPoint", ents->trigger.slabs[i].midPoint, allocator);
        json_trigger_slab.AddMember(
            "halfSize", ents->trigger.slabs[i].halfSize, allocator);
      ...
      for (auto i = 0; i < ents->stageCount; i++)
      {
        ...
        json_stage.AddMember(
            "origin", float_array_to_json(stage->origin, 3), allocator);
        json_stage.AddMember("triggerIndex", stage->triggerIndex, allocator);
        json_stage.AddMember(
            "sunPrimaryLightIndex", stage->sunPrimaryLightIndex, allocator);
    ...
    {
      output.AddMember(
          "mapEnts", rapidjson::Value(rapidjson::kNullType), allocator);
    }
    ...
    for (size_t i = 0; i < clip_map->smodelNodeCount; i++)
    {
      ...
      json_smodelnode.AddMember(
          "bounds", bounds_to_json(smodelNode->bounds), allocator);
      json_smodelnode.AddMember(
          "firstChild", smodelNode->firstChild, allocator);
      json_smodelnode.AddMember(
          "childCount", smodelNode->childCount, allocator);
    ...
    for (size_t i = 0; i < ARRAYSIZE(clip_map->dynEntCount); i++)
    {
      ...
      if (def_list)
      {
        ...
        for (size_t j = 0; j < clip_map->dynEntCount[i]; j++)
        {
          ...
          json_dyn_entity_def.AddMember(
              "pose", placement_to_json(def->pose), allocator);
          json_dyn_entity_def.AddMember(
              "xModel",
              def->xModel ? RAPIDJSON_STR(def->xModel->name)
                          : rapidjson::Value(rapidjson::kNullType),
              allocator);
          json_dyn_entity_def.AddMember(
              "brushModel", def->brushModel, allocator);
          json_dyn_entity_def.AddMember(
              "physicsBrushModel", def->physicsBrushModel, allocator);
          json_dyn_entity_def.AddMember(
              "destroyFx",
              def->destroyFx ? RAPIDJSON_STR(def->destroyFx->name)
                             : rapidjson::Value(rapidjson::kNullType),
              allocator);
          json_dyn_entity_def.AddMember(
              "physPreset",
              def->physPreset ? RAPIDJSON_STR(def->physPreset->name)
                              : rapidjson::Value(rapidjson::kNullType),
              allocator);
          json_dyn_entity_def.AddMember("health", def->health, allocator);

          rapidjson::Value json_mass(rapidjson::kObjectType);
          json_mass.AddMember("centerOfMass",
                              float_array_to_json(def->mass.centerOfMass, 3),
                              allocator);
          json_mass.AddMember(
              "momentsOfInertia",
              float_array_to_json(def->mass.momentsOfInertia, 3),
              allocator);
          json_mass.AddMember(
              "productsOfInertia",
              float_array_to_json(def->mass.productsOfInertia, 3),
              allocator);
          json_dyn_entity_def.AddMember("mass", json_mass, allocator);

          json_dyn_entity_def.AddMember("contents", def->contents, allocator);

          json_dyn_entity_def_pack.AddMember(
              "dynEntityDef", json_dyn_entity_def, allocator);
          ...
          // json_dyn_entity_pose.AddMember("pose",
          // placement_to_json(pose->pose), allocator);
          // json_dyn_entity_pose.AddMember("radius", pose->radius, allocator);
          // json_dyn_entity_def_pack.AddMember("dynEntPose",
          // json_dyn_entity_pose, allocator);

          // rapidjson::Value json_dyn_entity_client(rapidjson::kObjectType);
          // auto client = &clip_map->dynEntClientList[i][j];
          // json_dyn_entity_client.AddMember("physObjId", client->physObjId,
          // allocator); json_dyn_entity_client.AddMember("flags",
          // client->flags, allocator);
          // json_dyn_entity_client.AddMember("lightingHandle",
          // client->lightingHandle, allocator);
          // json_dyn_entity_client.AddMember("health", client->health,
          // allocator); json_dyn_entity_def_pack.AddMember("dynEntClient",
          // json_dyn_entity_client, allocator);
          ...
          // json_dyn_entity_coll.AddMember("nextEntInSector",
          // coll->nextEntInSector, allocator);
          // json_dyn_entity_coll.AddMember("linkMins",
          // float_array_to_json(coll->linkMins, 2), allocator);
          // json_dyn_entity_coll.AddMember("linkMaxs",
          // float_array_to_json(coll->linkMaxs, 2), allocator);
          // json_dyn_entity_def_pack.AddMember("dynEntColl",
          // json_dyn_entity_coll, allocator);
          //

          json_dyn_entity_def_list.PushBack(json_dyn_entity_def_pack,
                                            allocator);
        }
      ...
      {
        json_dyn_entities.PushBack(rapidjson::Value(rapidjson::kNullType),
                                   allocator);
      }
    }
