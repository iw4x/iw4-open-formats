...
{
    std::vector<native::XAsset> ilightdef::get_child_assets(const native::XAssetHeader& header) const
    {
        std::vector<native::XAsset> result{};
        const auto asset = header.lightDef;

        result.push_back({native::XAssetType::ASSET_TYPE_IMAGE, { asset->attenuation.image } });
