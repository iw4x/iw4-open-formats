...
#define IW4X_PHYSPRESET_VERSION 1
  ...
  bool iphyspreset::write_internal(const native::XAssetHeader& header) const
  {
    ...
    output.AddMember(
        "explosiveForceScale", asset->explosiveForceScale, allocator);
    output.AddMember(
        "sndAliasPrefix", RAPIDJSON_STR(asset->sndAliasPrefix), allocator);
    output.AddMember(
        "piecesSpreadFraction", asset->piecesSpreadFraction, allocator);
    output.AddMember(
        "piecesUpwardVelocity", asset->piecesUpwardVelocity, allocator);
    output.AddMember(
        "tempDefaultToCylinder", asset->tempDefaultToCylinder, allocator);
    output.AddMember(
        "perSurfaceSndAlias", asset->perSurfaceSndAlias, allocator);
  ...
  void* iphyspreset::read_internal(const std::string& name) const
  {
    ...
    if (utils::io::file_exists(path))
    {
      ...
      try
      {
        asset->name = local_allocator.duplicate_string(
            physPresetJson["name"].GetString());
        ...
        asset->bulletForceScale =
            physPresetJson["bulletForceScale"].Get<float>();
        asset->explosiveForceScale =
            physPresetJson["explosiveForceScale"].Get<float>();
        asset->sndAliasPrefix = local_allocator.duplicate_string(
            physPresetJson["sndAliasPrefix"].GetString());
        asset->piecesSpreadFraction =
            physPresetJson["piecesSpreadFraction"].Get<float>();
        asset->piecesUpwardVelocity =
            physPresetJson["piecesUpwardVelocity"].Get<float>();
        asset->tempDefaultToCylinder =
            physPresetJson["tempDefaultToCylinder"].Get<bool>();
        asset->perSurfaceSndAlias =
            physPresetJson["perSurfaceSndAlias"].Get<bool>();
  ...
  std::filesystem::path iphyspreset::get_file_name(
