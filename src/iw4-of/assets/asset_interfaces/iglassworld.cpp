...
namespace iw4of::interfaces
{
  ...
  bool iglassworld::write_internal(const native::XAssetHeader& header) const
  {
    ...
    for (size_t i = 0; i < asset->g_glassData->pieceCount; i++)
    {
      ...
      piece.AddMember(
          "lastStateChangeTime", iwPiece.lastStateChangeTime, allocator);
    ...
    glassData.AddMember(
        "damageToWeaken", asset->g_glassData->damageToWeaken, allocator);
    glassData.AddMember(
        "damageToDestroy", asset->g_glassData->damageToDestroy, allocator);
  ...
  void* interfaces::iglassworld::read_internal(const std::string& name) const
  {
    ...
    if (utils::io::file_exists(path))
    {
      ...
      auto version = gameWorldJson["version"].IsNumber()
                         ? gameWorldJson["version"].Get<int32_t>()
                         : 0;
      if (version != IW4X_GAMEWORLD_VERSION)
      {
        print_error(
            "Invalid GameWorld json version for {}, expected {} and got {}\n",
            name,
            IW4X_GAMEWORLD_VERSION,
            version);
        return nullptr;
      }
      ...
      asset->name =
          local_allocator.duplicate_string(gameWorldJson["name"].GetString());
      ...
      if (gameWorldJson["glassData"].IsObject())
      {
        ...
        try
        {
          glassData->damageToDestroy =
              jsonGlassData["damageToDestroy"].Get<uint16_t>();
          glassData->damageToWeaken =
              jsonGlassData["damageToWeaken"].Get<uint16_t>();
          ...
          if (jsonGlassData["glassNames"].IsArray())
          {
            ...
            glassData->glassNames =
                local_allocator.allocate_array<native::G_GlassName>(
                    glassData->glassNameCount);

            for (size_t i = 0; i < glassData->glassNameCount; i++)
            {
              const auto& jsonGlassName = glassNames[i];
              glassData->glassNames[i].nameStr =
                  local_allocator.duplicate_string(
                      jsonGlassName["nameStr"].GetString());

              glassData->glassNames[i].name =
                  jsonGlassName["name"].Get<uint16_t>();

              if (jsonGlassName["piecesIndices"].IsArray())
              {
                const auto& jsonPiecesIndices = jsonGlassName["piecesIndices"];
                glassData->glassNames[i].pieceCount =
                    static_cast<uint16_t>(jsonPiecesIndices.Size());
                glassData->glassNames[i].pieceIndices =
                    local_allocator.allocate_array<uint16_t>(
                        glassData->glassNames[i].pieceCount);
                for (size_t j = 0; j < glassData->glassNames[i].pieceCount; j++)
                {
                  glassData->glassNames[i].pieceIndices[j] =
                      jsonPiecesIndices[j].Get<uint16_t>();
                }
              }
            }
          }
          ...
          if (jsonGlassData["glassPieces"].IsArray())
          {
            ...
            glassData->glassPieces =
                local_allocator.allocate_array<native::G_GlassPiece>(
                    glassData->pieceCount);

            for (size_t i = 0; i < glassData->pieceCount; i++)
            {
              glassData->glassPieces[i].collapseTime =
                  glassPieces[i]["collapseTime"].Get<uint16_t>();
              glassData->glassPieces[i].damageTaken =
                  glassPieces[i]["damageTaken"].Get<uint16_t>();
              glassData->glassPieces[i].lastStateChangeTime =
                  glassPieces[i]["lastStateChangeTime"].Get<int32_t>();
              glassData->glassPieces[i].impactDir =
                  glassPieces[i]["impactDir"].Get<char>();
  ...
  std::filesystem::path interfaces::iglassworld::get_file_name(
